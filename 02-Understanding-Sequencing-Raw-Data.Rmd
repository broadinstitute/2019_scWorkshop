...
```{bash, eval = FALSE}
  ...
  - -it: flag to run the container interactively
    ...
    -v $PWD:/home: map your home directory to a directory inside docker container called home
...
#### Viewing entire file
```{bash, eval = FALSE}
cat Teichmann_2i_2_2_2.fastq
```

#### Viewing first 10 lines

```{bash, eval = FALSE}
head Teichmann_2i_2_2_2.fastq
```
...
```{bash, eval = FALSE}
samtools view pbmc_1k_protein_v3_possorted_genome_bam.bam | head
```

#### Stream Viewing with less command

```{bash, eval = FALSE}
samtools view pbmc_1k_protein_v3_possorted_genome_bam.bam | less
```
